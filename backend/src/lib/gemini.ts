...
function getAi(): GoogleGenAI {
  if (!baseAiInstance) {
    ...
    if (!apiKey) {
      console.warn(
        "⚠️ [Gemini Client] GEMINI_API_KEY is not defined in process.env!",
      );
    }
...
async function withRetry<T>(
  ...
  try {
    ...
    if (isRetryable && retries > 0) {
      console.warn(
        `⚠️ [Gemini API] Call failed (${err.message || err}). Retrying in ${delay}ms... (${retries} retries left)`,
      );
...
export async function withAbort(
  promise: Promise<any>,
  signal?: AbortSignal,
): Promise<any> {
...
export const ai = {
  models: {
    generateContent: (args: any) =>
      withRetry(() => getAi().models.generateContent(args)),
    generateContentStream: (args: any) =>
      withRetry(() => getAi().models.generateContentStream(args)),
    embedContent: (args: any) =>
      withRetry(() => getAi().models.embedContent(args)),
  },
  chats: {
    create: (args: any) => {
      const chat = getAi().chats.create(args);
      const originalSendMessage = chat.sendMessage.bind(chat);
      chat.sendMessage = (msgArgs: any) =>
        withRetry(() => originalSendMessage(msgArgs));
      return chat;
    },
  },
} as any;
